...
public data class AccountBalanceRequest(
   ...
   * @param securityCredential Base64 encoded string of the M-PESA short code and password, which is encrypted using M-PESA public key and validates the transaction on M-PESA Core system. It indicates the Encrypted credential of the initiator getting the account balance. Its value must match the inputted value of the parameter IdentifierType.
   * @param partyA The shortcode of the organization querying for the account balance. Sample: 60072
   * @param accountBalanceOrganizationType Type of organization querying for the account balance. Sample: [AccountBalanceOrganizationType.Shortcode] or [AccountBalanceOrganizationType.TillNumber]
   * @param resultUrl It indicates the destination URL which Daraja should send the result message to.
   * @param queueTimeOutUrl The end-point that receives a timeout message.
   * @param remarks Comments that are sent along with the transaction. Sample: tests
   */
